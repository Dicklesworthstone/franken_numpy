...
| Random (numpy.random) | parity_green | PCG64DXSM generator with oracle-verified distributions; no-seed constructors source OS entropy like NumPy; Lemire bounded integers + buffered uint32; BTPE binomial + inversion; HRUA hypergeometric + direct; PTRS Poisson + multiplicative; NumPy-exact gamma; zipf with Umin clamping; oracle and reproducibility coverage green | — |
...
|---|---|---|
| fnp-ufunc | 2,191 | Core array operations, math, sorting, polynomials (Chebyshev div/roots/fromroots, Legendre/Hermite/Laguerre div/roots/fromroots edge cases), reductions, oracle tests, linalg bridge, FFT (hfft/ihfft), hermfit/lagfit, masked cov/corrcoef, datetime parsing, gufunc validation, parameter parity (equal_nan, bitorder, mode, endpoint, trim, period, axes, prepend/append, left/right), r_/c_ concat helpers, GridSpec mgrid/ogrid, linspace_retstep, concatenate_flat, einsum coverage, NaN/Inf/signed-zero edge cases (maximum, heaviside, logaddexp, floor_divide, remainder, clip, divmod, cummin/cummax), NaN set-op parity, `compress` short-mask truncation parity, behavioral edge cases |
| fnp-ndarray | 221 | Shape legality, stride calculus, broadcast contracts, overlap detection, multi-axis negative strides, broadcast/reshape/stride edge cases, F-order, required_view_nbytes |
| fnp-linalg | 308 | Linear algebra decompositions, solvers, norms, batch ops (det/inv/solve/trace), 16 NumPy oracle tests, extreme-scale regression, non-finite parity (cond_p, cross_product 2D, NaN/Inf propagation), rectangular MxN norm/cond support, cond_mxn NaN handling |
| fnp-random | 310 | RNG distributions with statistical conformance coverage, permuted (1D/2D/axis/deterministic), seeding, reproducibility, large-n binomial/multinomial |
| fnp-iter | 200 | Transfer-loop selector, NDIter traversal/broadcast/overlap contracts, stateful `Nditer` wrapper (`iterindex`/`multi_index`/reset/seek/external-loop chunks), flatiter indexing/assignment, ndindex/ndenumerate iterators |
...
| fnp-dtype | 257 | Dtype taxonomy, promotion table (all 324 pairs explicit), cast policy primitives, NumPy byte-width parsing |
| fnp-python | 2,127 | PyO3 package surface coverage for `frompyfunc`, `vectorize`, `digitize`, `searchsorted`, `interp`, `where`, `flatnonzero`, `argwhere`, `count_nonzero`, `isposinf`, `isneginf`, `signbit`, `isnan`, `isinf`, `isfinite`, `spacing`, `sign`, `floor`, `ceil`, `trunc`, `rint`, `degrees`, `radians`, `sinc`, `copysign`, `nextafter`, `hypot`, `ldexp`, `logaddexp`, `logaddexp2`, `frexp`, `modf`, `nan_to_num`, `take`, `put`, `take_along_axis`, `put_along_axis`, `compress`, `extract`, `place`, `putmask`, `split`, `array_split`, `hsplit`, `vsplit`, `dsplit`, `vstack`, `hstack`, `dstack`, `column_stack`, `indices`, `diag`, `diagflat`, `diagonal`, `fill_diagonal`, `ix_`, `meshgrid`, `mgrid`, `ogrid`, `r_`, `c_`, `ravel_multi_index`, `unravel_index`, `diag_indices`, `diag_indices_from`, `tril_indices`, `tril_indices_from`, `triu_indices`, `triu_indices_from`, `select`, and `choose`, including live callable parity, sorter/side bridging, left/right fill parity, one-argument nonzero parity, flat/coordinate nonzero wrappers, `count_nonzero(axis, keepdims)` parity, `isposinf`/`isneginf`/`signbit`/`isnan`/`isinf`/`isfinite`/`spacing`/`sign`/`floor`/`ceil`/`trunc`/`rint`/`degrees`/`radians`/`sinc`/`copysign`/`nextafter`/`hypot`/`ldexp`/`logaddexp`/`logaddexp2`/`frexp`/`modf`/`nan_to_num` oracle parity, `take(axis)` reshape/scalar semantics, in-place `put`/`place`/`putmask`/`fill_diagonal` copy-back semantics, `split`/`array_split` integer-section parity with NumPy index-sequence fallback, `hsplit`/`vsplit`/`dsplit` dimensionality guards and list-output parity, `vstack`/`hstack` scalar-promotion parity with generator rejection and NumPy `dtype=`/`casting=` fallback, `dstack`/`column_stack` scalar/object fallback parity, `indices(dtype)` grid parity, `diag` vector/matrix offset parity, `diagflat` flatten/offset parity with precise dtype preservation, `diagonal(offset, axis1, axis2)` parity across arbitrary axes, `fill_diagonal(wrap)` rectangular and equal-length N-D parity with cyclic value assignment, `ix_` bool/nonzero normalization and empty-input dtype parity, dense/sparse `meshgrid(*xi, copy, indexing)` parity including empty, scalar, flattened, object, and writeback-view behavior, `mgrid[...]` dense slice-object parity including single-slice vs tuple shape, defaulted bounds, and float32/complex-count dtypes, `ogrid[...]` sparse slice-object parity including tuple return shape and dtype preservation, `r_[...]` Rust-backed numeric/scalar/slice parity with NumPy directive fallback, `c_[...]` column-stack parity with 1-D promotion plus directive/object fallback, `ravel_multi_index(mode, order)` parity including empty and scalar cases, `unravel_index(order)` parity including 0-D rejection and shape-preserving integer outputs, diagonal/triangle index tuple parity, shape-only `*_from` wrappers for arbitrary ndarray dtypes, `axis=None` flatten semantics for `take_along_axis` and `put_along_axis`, overlap-safe view mutation parity, short-condition truncation parity, first-match `select` behavior, large-uint64 coverage, full `choose(mode='raise'|'wrap'|'clip')` parity, integer exponent tuple output for `frexp`, signed fractional/integral tuple parity for `modf`, scalar replacement parity for `nan_to_num`, and module export wiring |
| fnp-runtime | 131 | Mode split, fail-closed decoding, override-audit gate, risk-aware decision engine, evidence ledger |
| **Total** | **6,392** (05-17 count; **8,688** `#[test]` functions on 2026-09-02, per-crate rows above not refreshed: fnp-python 3,635, fnp-ufunc 2,471, fnp-random 477, fnp-linalg 459, fnp-io 407, fnp-conformance 395, fnp-dtype 276, fnp-ndarray 231, fnp-iter 203, fnp-runtime 134) | Sum across all 10 crates (re-verified 2026-05-17 via `cargo test --workspace`, unchanged from the 2026-05-16 baseline; `cargo check --workspace` re-verified clean 2026-05-20 in ~37 s on a fresh target dir, after the Cargo.toml workspace-deps refactor and 7-fuzz-toolchain-pin additions; bead tracker stands at 1,417 closed beads as of 2026-05-19 (was 1,337 on 2026-05-16); underlying Rust surface is 1,575 `pub fn` declarations across `crates/*/src/**/*.rs` — see README's `br` row in "Multi-Agent Development Process" for the live-count grep recipe and `audit_numpy_reality.md` for the pub-fn scan command). |
...
1. **Python packaging/distribution.** `pyproject.toml` + maturin landed 2026-09-03 (`pip install .` builds and installs a working wheel; Linux / CPython 3.13 verified). Still missing: a PyPI release, macOS and Windows wheels, a CI packaging job. This is the difference between "library that exposes the full surface" and "drop-in NumPy replacement" — the surface itself is complete and CI-locked.
