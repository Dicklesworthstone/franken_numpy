...
| **Total** | **6,392** | Sum across all 10 crates (re-verified 2026-05-17 via `cargo test --workspace`, unchanged from the 2026-05-16 baseline; bead tracker stands at 1,337 closed beads on the same date; underlying Rust surface is 1,575 `pub fn` declarations across `crates/*/src/**/*.rs` — see README's `br` row in "Multi-Agent Development Process" for the live-count grep recipe and `audit_numpy_reality.md` for the pub-fn scan command). |
