...
| script | purpose | how to run | last validated |
|---|---|---|---|
| `run_numpy_dropin_suite.sh` + `numpy_dropin_plugin.py` | runs numpy's OWN test modules twice — A/A lane (real numpy) and swap lane (fnp_python in numpy's place: public names resolve on fnp, private ones on numpy) — and lists every test that passes on numpy but fails with fnp. Each is a drop-in defect candidate. | `PYTHON=<python with numpy+pytest+hypothesis> scripts/run_numpy_dropin_suite.sh <fnp_python.so> <out_dir> [numpy.test.module ...]` | 2026-09-26, local .so at 60473602 (+ the vars() stand-in fix), numpy 2.4.3: 122 modules, 47,238 A/A-passing tests, 56 divergences before cc229e30's dir fix (about 53 after), every one a by-design row the plugin docstring names - ufunc/function identity handed to override hooks, `__module__` identity, numpy-written pickles, ctypes, version. History: 2026-09-24 at f11c7752, 24 modules, 421 divergences; 2026-09-25 at ccee9d2d, 57 modules, 31; the fixes these runs drove are the deadlock-audit-rc0923-epic-71qy3.8 commits |
