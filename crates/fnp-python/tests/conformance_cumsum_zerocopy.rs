...
#[test]
fn cumsum_zerocopy_f16_per_step_narrow_bit_exact() -> Result<(), String> {
    // numpy's f16 cumsum narrows the accumulator to f16 EVERY step (the
    // wide-f32-accumulator alternative mismatches 6/6 in the discriminator);
    // the zero-copy path replays out[0]=a[0]; out[i]=f16(f32(out[i-1])+f32(a[i])).
    let script = fnp_script(
        r#"
verdicts = []
rng = np.random.default_rng(20260715)
for n in (1000, 100000, 2_000_003):
    for scale in (0.3, 30.0):
        a = (rng.standard_normal(n) * scale).astype(np.float16)
        r = fnp.cumsum(a); e = np.cumsum(a)
        if r.dtype != e.dtype or r.tobytes() != e.tobytes():
            verdicts.append(f"FAIL n={n} scale={scale}")
# specials: leading -0.0 verbatim, inf/nan propagation, overflow saturation
a = (rng.standard_normal(100000) * 0.3).astype(np.float16)
a[0] = np.float16(-0.0); a[7] = np.float16(np.inf); a[11] = np.float16(np.nan)
a[13] = np.float16(60000); a[14] = np.float16(60000)
r = fnp.cumsum(a); e = np.cumsum(a)
ok = bool(((r.view(np.uint16) == e.view(np.uint16)) | (np.isnan(r) & np.isnan(e))).all())
if not ok:
    verdicts.append("FAIL specials")
# 2-D axis=None flattens (native), per-axis defers byte-exactly
m = (rng.standard_normal((300, 400)) * 0.3).astype(np.float16)
if fnp.cumsum(m).tobytes() != np.cumsum(m).tobytes():
    verdicts.append("FAIL 2d flatten")
if fnp.cumsum(m, axis=1).tobytes() != np.cumsum(m, axis=1).tobytes():
    verdicts.append("FAIL 2d axis defer")
if fnp.cumsum(a[::2]).tobytes() != np.cumsum(a[::2]).tobytes():
...
"#
    ...
    assert_eq!(
        result.trim(),
        "True",
        "f16 zero-copy cumsum must replay numpy's per-step-narrow chain byte-exactly: {result}"
    );
    Ok(())
}
