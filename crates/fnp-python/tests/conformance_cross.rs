...
a = np.array([1, 2])
b = np.array([3, 4])
# numpy 2.4.3 computes the scalar z component; numpy 2.5.2 REMOVED the 2-vector cross and
# raises ValueError. The contract is therefore "whatever numpy does", not a fixed value, so
# this compares BEHAVIOUR - and it is the assertion that caught fnp.cross still computing
# where the live incumbent refuses.
def attempt(module):
    try:
