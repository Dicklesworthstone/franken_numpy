...
//!
//! The large-array probes now live in the companion shard
//! `conformance_unravel_unique_wide.rs`: `wide_int_setops_bit_exact_match_numpy`,
//! `add_at_i64_large_target_parallel_matches_numpy`,
//! `int_sort_class_stale_basis_probe_and_parity`, `setops_nd_flat_view_matches_numpy` and
//! `unique_nd_flat_view_matches_numpy`. Their array sizes are LOAD-BEARING: they exist to
//! cross the native parallel dispatch gates, so shrinking them would move the probe onto
//! the serial path and stop it testing what it exists to test. They were separated rather
//! than shrunk or ignored; both shards run by default and coverage is unchanged. Split
//! under bead `deadlock-audit-syi8e`.
//!
//! `wide_int_setops_bit_exact_match_numpy` was NOT on the bead's list and is the single
//! largest cost in the family: 115.372s of a 146.26s parent wall on rch worker hz2,
//! 2026-08-15, `cargo test --report-time` under default test parallelism. Moving only the
//! four originally-named tests left the parent over 120s. Re-measure before trusting any
//! per-test ranking in this file, including this one.
//!
//! Keep this shard's own work small: a new probe sized to cross a parallel dispatch gate
//! belongs in the `_wide` companion.
//!
//! RUNTIME IS HOST-SCOPED — do not quote a number without naming the host. A sibling shard
//! measured 9.03s, 183.46s and 617.62s for identical tests on three different hosts. And
...
//!
//! KNOWN PRE-EXISTING FAILURE, not this shard's fault: `f16_unique_presence_table_bit_exact`
//! fails on AVX-512 hosts (hz1/hz2) because `np.unique(float16)` sorts internally and hits
//! numpy's x86-simd-sort fp16 defect. Reproduced at base commit with the split reverted.
//! Tracked by bead `deadlock-audit-f7qjf`.
