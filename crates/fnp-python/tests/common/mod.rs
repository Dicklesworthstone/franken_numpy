...
// ---------------------------------------------------------------------------
// Locating the built extension for the suites that drive it through a REAL
// interpreter (`python3 -c ...`) rather than through the in-process harness above.
// ---------------------------------------------------------------------------
...
///
/// WHY THIS SEARCHES INSTEAD OF ASSUMING (`deadlock-audit-ovdn9`). Every suite that shells out
/// to `python3` used to compute the path as a SIBLING of its own test binary:
///
...
/// extension genuinely has not been built. `FNP_PYTHON_MODULE` overrides everything.
