...
//!
//! The three large-n packed-Latin-1 probes live in the companion shard
//! `conformance_setops_wide.rs`, which groups the wide-key string routes together. Both
//! shards run by default and coverage is unchanged. Separated under bead
//! `deadlock-audit-syi8e`.
//!
//! THIS SHARD HAS NO RUNTIME PROBLEM, and the bead's premise for it is REFUTED. It was
//! filed claiming those three probes each ran over a minute (2026-08-09). Re-measured
//! 2026-08-15 on rch worker hz2 with `cargo test --report-time`, they are the three
//! CHEAPEST tests in the whole family — 2.140s, 1.731s and 1.039s — and the parent
//! completes in 23.12s, nowhere near a 120s budget. The cost here is spread across roughly
//! thirteen tests at 4-19s each with no dominator. The likely reconciliation is that the
//! wide-key string set-op work landed in between and closed those probes. The separation is
//! therefore CATEGORICAL, not a performance fix — do not cite it as one.
//!
//! Element count is not the tell here either: several tests that stay behind also build
//! 300,000-element inputs and are the expensive ones. Re-measure before trusting any
