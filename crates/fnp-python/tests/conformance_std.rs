...
fn fnp_std_script(body: String) -> String {
...
fn std_python_container_keyword_outcomes_match_numpy() -> Result<(), String> {
    let cases = [
        ("list input scalar", "result = MODULE.std([1, 2, 3])"),
        (
...
result = MODULE.std(np.array([[1.0, 2.0], [3.0, 4.0]]), axis=0, out=out)",
        ...
        ("axis error type", "result = MODULE.std([1, 2, 3], axis=2)"),
    ];
...
#[test]
fn std_flat_matches_numpy_across_50_cases() -> Result<(), String> {
    let test_cases = vec![
        // Basic arrays
        "[1, 2, 3]",
        "[1, 2, 3, 4, 5]",
...
#[test]
fn std_2d_axis_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
    ...
    for (arr_str, axis) in &test_cases {
        let script =
            format!("import numpy as np; print(np.std(np.array({arr_str}), axis={axis}).tolist())");
        let numpy_result = numpy_oracle(&script)?;
        let numpy_vals = parse_float_list(&numpy_result);

        let rust_script = fnp_std_script(format!(
            "print(fnp.std(np.array({arr_str}), axis={axis}).tolist())"
        ));
        let rust_result = numpy_oracle(&rust_script)?;
...
#[test]
fn std_3d_axis_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
        // 3D arrays
        ("[[[1, 2], [3, 4]], [[5, 6], [7, 8]]]", "0"),
        ("[[[1, 2], [3, 4]], [[5, 6], [7, 8]]]", "1"),
    ...
    for (arr_str, axis) in &test_cases {
        let script = format!(
            "import numpy as np; print(np.std(np.array({arr_str}), axis={axis}).flatten().tolist())"
        );
        let numpy_result = numpy_oracle(&script)?;
        let numpy_vals = parse_float_list(&numpy_result);

        let rust_script = fnp_std_script(format!(
            "print(fnp.std(np.array({arr_str}), axis={axis}).flatten().tolist())"
        ));
        let rust_result = numpy_oracle(&rust_script)?;
        let rust_vals = parse_float_list(&rust_result);
...
#[test]
fn std_keepdims_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
        // 1D with keepdims
        ("[1, 2, 3, 4, 5]", "None", true),
    ...
    for (arr_str, axis, keepdims) in &test_cases {
        ...
        let rust_script = fnp_std_script(format!(
            "print(fnp.std(np.array({arr_str}){axis_arg}, keepdims={}).shape)",
            if *keepdims { "True" } else { "False" }
        ));
        let rust_result = numpy_oracle(&rust_script)?;
...
#[test]
fn std_ddof_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
    ...
    for (arr_str, ddof) in &test_cases {
        ...
        let rust_script =
            fnp_std_script(format!("print(fnp.std(np.array({arr_str}), ddof={ddof}))"));
        let rust_result = numpy_oracle(&rust_script)?;
        let rust_val = parse_float(&rust_result);
...
#[test]
fn std_non_integral_ddof_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
        ("[1.0, 2.0, 3.0]", "-1"),
        ("[1.0, 2.0, 3.0]", "0.5"),
        ("[1.0, 2.0, 3.0]", "1.5"),
...
#[test]
fn std_integer_dtypes_match_numpy() -> Result<(), String> {
    let test_cases = vec![
    ...
    for (arr_expr, axis) in &test_cases {
        ...
        let rust_script = fnp_std_script(format!("print(float(fnp.std({arr_expr}{axis_arg})))"));
        let rust_result = numpy_oracle(&rust_script)?;
        let rust_val = parse_float(&rust_result);
...
#[test]
fn std_nan_handling_matches_numpy() -> Result<(), String> {
    let test_cases = vec![
        "[1.0, np.nan, 3.0]",
        "[np.nan, 2.0, 3.0]",
        "[1.0, 2.0, np.nan]",
