...
fn main() {
    ...
    let mut g = Generator::from_pcg64_dxsm(seed).unwrap();
    println!("noncentral_chisquare: {:?}", g.noncentral_chisquare(2.0, 3.0, 5).unwrap());
    
    let mut g = Generator::from_pcg64_dxsm(seed).unwrap();
    println!("noncentral_f: {:?}", g.noncentral_f(2.0, 3.0, 4.0, 5).unwrap());
    ...
    let mut g = Generator::from_pcg64_dxsm(seed).unwrap();
    println!("multivariate_hypergeometric: {:?}", g.multivariate_hypergeometric(&[10, 20, 30], 5, 2));
}
