...
const SUBTASK_ID: &str = "FNP-P2C-007-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet007_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet007_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let packet_dir = repo_root.join("artifacts/phase2c/FNP-P2C-007");
    ...
    let parity_report = ParityReport {
        ...
        divergence_classes: vec![
            "no_observed_strict_drift_in_packet007_gates".to_string(),
            "no_observed_hardened_drift_in_packet007_gates".to_string(),
        ],
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-007-maintainers".to_string(),
            follow_up_actions: vec![
                "Expand packet-007 workflow scenario breadth beyond the current RNG replay and hostile guardrail pair before readiness drill sign-off."
                    .to_string(),
                "Keep packet-007 calibration thresholds aligned with full RNG corpus growth and refresh optimization/isomorphism evidence when fixture-lookup policy changes."
                    .to_string(),
            ],
        ...
        quality_gates: vec![
            ...
            QualityGate {
                command: "rch exec -- cargo test -p fnp-random".to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        packet_id: PACKET_ID.to_string(),
        oracle_tests: vec![
            "legacy_numpy_code/numpy/numpy/random/tests/test_seed_sequence.py".to_string(),
            "legacy_numpy_code/numpy/numpy/random/tests/test_generator_mt19937.py".to_string(),
            "legacy_numpy_code/numpy/numpy/random/tests/test_random.py".to_string(),
            "legacy_numpy_code/numpy/numpy/random/tests/test_generator_mt19937_regressions.py"
                .to_string(),
        ],
        fixtures: vec![
            FixtureEntry {
                id: "p2c007_rng_differential".to_string(),
                input_path: "crates/fnp-conformance/fixtures/rng_differential_cases.json"
                    .to_string(),
                oracle_case_id: "rng_differential_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c007_rng_metamorphic".to_string(),
                input_path: "crates/fnp-conformance/fixtures/rng_metamorphic_cases.json"
                    .to_string(),
                oracle_case_id: "rng_metamorphic_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c007_rng_adversarial".to_string(),
                input_path: "crates/fnp-conformance/fixtures/rng_adversarial_cases.json"
                    .to_string(),
                oracle_case_id: "rng_adversarial_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c007_rng_workflow_scenarios".to_string(),
                input_path: "crates/fnp-conformance/fixtures/workflow_scenario_corpus.json"
                    .to_string(),
                oracle_case_id: "rng_packet_workflow_contract".to_string(),
            },
        ],
    }
}
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    ...
    paths.extend([
        ...
        repo_root.join("crates/fnp-conformance/fixtures/rng_metamorphic_cases.json"),
