...
const SUBTASK_ID: &str = "FNP-P2C-001-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet001_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet001_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let packet_dir = repo_root.join("artifacts/phase2c/FNP-P2C-001");
    ...
    let parity_report = ParityReport {
        ...
        divergence_classes: vec![
            "no_observed_strict_drift_in_packet001_gates".to_string(),
            "no_observed_hardened_drift_in_packet001_gates".to_string(),
        ],
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-001-maintainers".to_string(),
            follow_up_actions: vec![
                "Expand packet-001 workflow scenario breadth beyond the current shape replay and hostile guardrail pair before readiness drill sign-off."
                    .to_string(),
                "Keep packet-001 calibration thresholds aligned with full reshape/broadcast corpus growth and re-run optimization/isomorphism evidence when shape-validation policy changes."
                    .to_string(),
            ],
        ...
        quality_gates: vec![
            ...
            QualityGate {
                command: "rch exec -- cargo test -p fnp-ndarray".to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
            ...
            QualityGate {
                command:
                    "rch exec -- cargo run -p fnp-conformance --bin run_workflow_scenario_gate -- --log-path artifacts/phase2c/FNP-P2C-001/workflow_scenario_packet001_opt_e2e.jsonl --artifact-index-path artifacts/phase2c/FNP-P2C-001/workflow_scenario_packet001_opt_artifact_index.json --report-path artifacts/phase2c/FNP-P2C-001/workflow_scenario_packet001_opt_reliability.json --retries 1 --flake-budget 0 --coverage-floor 1.0"
                        .to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        packet_id: PACKET_ID.to_string(),
        oracle_tests: vec![
            "legacy_numpy_code/numpy/numpy/_core/tests/test_shape_base.py".to_string(),
            "legacy_numpy_code/numpy/numpy/lib/tests/test_shape_base.py".to_string(),
        ],
        fixtures: vec![
            FixtureEntry {
                id: "p2c001_shape_stride_differential".to_string(),
                input_path: "crates/fnp-conformance/fixtures/shape_stride_cases.json".to_string(),
                oracle_case_id: "shape_stride_differential_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c001_shape_stride_metamorphic".to_string(),
                input_path: "crates/fnp-conformance/fixtures/shape_stride_cases.json".to_string(),
                oracle_case_id: "shape_stride_metamorphic_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c001_shape_stride_adversarial".to_string(),
                input_path: "crates/fnp-conformance/fixtures/shape_stride_cases.json".to_string(),
                oracle_case_id: "shape_stride_adversarial_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c001_shape_workflow_scenarios".to_string(),
                input_path: "crates/fnp-conformance/fixtures/workflow_scenario_corpus.json"
                    .to_string(),
                oracle_case_id: "shape_packet_workflow_contract".to_string(),
            },
        ],
    }
}
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    vec![
        ...
        packet_dir.join("workflow_scenario_packet001_opt_e2e.jsonl"),
        packet_dir.join("workflow_scenario_packet001_opt_reliability.json"),
        packet_dir.join("workflow_scenario_packet001_opt_artifact_index.json"),
        repo_root.join("crates/fnp-conformance/fixtures/shape_stride_cases.json"),
        repo_root.join(
            "crates/fnp-conformance/fixtures/oracle_outputs/shape_stride_differential_report.json",
        ),
    ]
}
