...
const SUBTASK_ID: &str = "FNP-P2C-008-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet008_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet008_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let packet_dir = repo_root.join("artifacts/phase2c/FNP-P2C-008");
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-008-maintainers".to_string(),
            follow_up_actions: vec![
                "Expand packet-008 workflow scenario breadth beyond the current linalg replay and hostile guardrail pair before readiness drill sign-off."
                    .to_string(),
                "Keep packet-008 calibration thresholds aligned with full linalg corpus growth and re-run optimization/isomorphism evidence when shape-validation policy changes."
                    .to_string(),
            ],
        ...
        quality_gates: vec![
            ...
            QualityGate {
                command: "rch exec -- cargo test -p fnp-linalg".to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
            ...
            QualityGate {
                command:
                    "rch exec -- cargo run -p fnp-conformance --bin run_workflow_scenario_gate -- --log-path artifacts/phase2c/FNP-P2C-008/workflow_scenario_packet008_opt_e2e.jsonl --artifact-index-path artifacts/phase2c/FNP-P2C-008/workflow_scenario_packet008_opt_artifact_index.json --report-path artifacts/phase2c/FNP-P2C-008/workflow_scenario_packet008_opt_reliability.json --retries 1 --flake-budget 0 --coverage-floor 1.0"
                        .to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        packet_id: PACKET_ID.to_string(),
        oracle_tests: vec![
            "legacy_numpy_code/numpy/numpy/linalg/tests/test_linalg.py".to_string(),
            "legacy_numpy_code/numpy/numpy/linalg/tests/test_regression.py".to_string(),
            "legacy_numpy_code/numpy/numpy/linalg/_linalg.py".to_string(),
            "legacy_numpy_code/numpy/numpy/linalg/umath_linalg.cpp".to_string(),
        ],
        fixtures: vec![
            FixtureEntry {
                id: "p2c008_linalg_differential".to_string(),
                input_path: "crates/fnp-conformance/fixtures/linalg_differential_cases.json"
                    .to_string(),
                oracle_case_id: "linalg_differential_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c008_linalg_metamorphic".to_string(),
                input_path: "crates/fnp-conformance/fixtures/linalg_metamorphic_cases.json"
                    .to_string(),
                oracle_case_id: "linalg_metamorphic_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c008_linalg_adversarial".to_string(),
                input_path: "crates/fnp-conformance/fixtures/linalg_adversarial_cases.json"
                    .to_string(),
                oracle_case_id: "linalg_adversarial_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c008_linalg_workflow_scenarios".to_string(),
                input_path: "crates/fnp-conformance/fixtures/workflow_scenario_corpus.json"
                    .to_string(),
                oracle_case_id: "linalg_packet_workflow_contract".to_string(),
            },
        ],
    }
}
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    vec![
        ...
        packet_dir.join("workflow_scenario_packet008_opt_e2e.jsonl"),
        packet_dir.join("workflow_scenario_packet008_opt_reliability.json"),
        packet_dir.join("workflow_scenario_packet008_opt_artifact_index.json"),
        repo_root.join("crates/fnp-conformance/fixtures/linalg_differential_cases.json"),
        repo_root.join("crates/fnp-conformance/fixtures/linalg_metamorphic_cases.json"),
        repo_root.join("crates/fnp-conformance/fixtures/linalg_adversarial_cases.json"),
        repo_root
            .join("crates/fnp-conformance/fixtures/oracle_outputs/linalg_differential_report.json"),
    ]
}
