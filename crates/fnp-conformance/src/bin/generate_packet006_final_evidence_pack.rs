...
const PACKET_ID: &str = "FNP-P2C-006";
const SUBTASK_ID: &str = "FNP-P2C-006-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet006_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet006_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let packet_dir = repo_root.join("artifacts/phase2c/FNP-P2C-006");
    ...
    let parity_report = ParityReport {
        ...
        divergence_classes: vec![
            "no_observed_strict_drift_in_packet006_gates".to_string(),
            "no_observed_hardened_drift_in_packet006_gates".to_string(),
        ],
        compatibility_drift_hash: "sha256:pending-p2c006-drift-hash".to_string(),
    };
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-006-maintainers".to_string(),
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        packet_id: PACKET_ID.to_string(),
        oracle_tests: vec![
            "legacy_numpy_code/numpy/numpy/lib/tests/test_stride_tricks.py".to_string(),
        ...
        fixtures: vec![
            FixtureEntry {
                id: "p2c006_shape_stride_differential".to_string(),
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    vec![
        ...
        packet_dir.join("workflow_scenario_packet006_opt_e2e.jsonl"),
        packet_dir.join("workflow_scenario_packet006_opt_reliability.json"),
        packet_dir.join("workflow_scenario_packet006_opt_artifact_index.json"),
