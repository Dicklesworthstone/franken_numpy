...
const SUBTASK_ID: &str = "FNP-P2C-004-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet004_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet004_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let packet_dir = repo_root.join("artifacts/phase2c/FNP-P2C-004");
    ...
    let parity_report = ParityReport {
        ...
        divergence_classes: vec![
            "no_observed_strict_drift_in_packet004_gates".to_string(),
            "no_observed_hardened_drift_in_packet004_gates".to_string(),
        ],
        compatibility_drift_hash: "sha256:pending-p2c004-drift-hash".to_string(),
    };
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-004-maintainers".to_string(),
        ...
        quality_gates: vec![
            ...
            QualityGate {
                command: "rch exec -- cargo test -p fnp-iter".to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        oracle_tests: vec![
            ...
            "legacy_numpy_code/numpy/numpy/_core/tests/test_indexing.py".to_string(),
            "legacy_numpy_code/numpy/numpy/lib/tests/test_index_tricks.py".to_string(),
            "legacy_numpy_code/numpy/numpy/_core/tests/test_multiarray.py".to_string(),
        ],
        fixtures: vec![
            FixtureEntry {
                id: "p2c004_iter_differential".to_string(),
                input_path: "crates/fnp-conformance/fixtures/iter_differential_cases.json"
                    .to_string(),
                oracle_case_id: "iter_differential_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c004_iter_metamorphic".to_string(),
                input_path: "crates/fnp-conformance/fixtures/iter_metamorphic_cases.json"
                    .to_string(),
                oracle_case_id: "iter_metamorphic_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c004_iter_adversarial".to_string(),
                input_path: "crates/fnp-conformance/fixtures/iter_adversarial_cases.json"
                    .to_string(),
                oracle_case_id: "iter_adversarial_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c004_iter_workflow_scenarios".to_string(),
                input_path: "crates/fnp-conformance/fixtures/workflow_scenario_corpus.json"
                    .to_string(),
                oracle_case_id: "nditer_packet_workflow_contract".to_string(),
            },
        ],
    }
}
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    vec![
        ...
        packet_dir.join("workflow_scenario_packet004_opt_e2e.jsonl"),
        packet_dir.join("workflow_scenario_packet004_opt_reliability.json"),
        packet_dir.join("workflow_scenario_packet004_opt_artifact_index.json"),
        repo_root.join("crates/fnp-conformance/fixtures/iter_differential_cases.json"),
        repo_root.join("crates/fnp-conformance/fixtures/iter_metamorphic_cases.json"),
        repo_root.join("crates/fnp-conformance/fixtures/iter_adversarial_cases.json"),
        repo_root
            .join("crates/fnp-conformance/fixtures/oracle_outputs/iter_differential_report.json"),
    ]
}
