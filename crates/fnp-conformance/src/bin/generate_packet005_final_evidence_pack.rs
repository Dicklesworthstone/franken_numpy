...
const SUBTASK_ID: &str = "FNP-P2C-005-I";
const REPRO_COMMAND: &str =
    "cargo run -p fnp-conformance --bin generate_packet005_final_evidence_pack";
...
fn main() {
    if let Err(err) = run() {
        eprintln!("generate_packet005_final_evidence_pack failed: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let final_pack = FinalEvidencePack {
        ...
        residual_risk_monitoring: ResidualRiskMonitoring {
            owner: "packet-005-maintainers".to_string(),
            follow_up_actions: vec![
                "Expand packet-005 workflow scenario breadth for override and gufunc edge families before readiness drill sign-off."
                    .to_string(),
        ...
        quality_gates: vec![
            ...
            QualityGate {
                command: "rch exec -- cargo test -p fnp-ufunc".to_string(),
                result: "pass".to_string(),
            },
            QualityGate {
...
fn build_fixture_manifest() -> FixtureManifest {
    FixtureManifest {
        ...
        oracle_tests: vec![
            ...
            "legacy_numpy_code/numpy/numpy/_core/tests/test_umath.py".to_string(),
            "legacy_numpy_code/numpy/numpy/_core/tests/test_overrides.py".to_string(),
            "legacy_numpy_code/numpy/numpy/_core/tests/test_multiarray.py".to_string(),
        ],
        fixtures: vec![
            FixtureEntry {
                id: "p2c005_ufunc_differential".to_string(),
                input_path: "crates/fnp-conformance/fixtures/ufunc_input_cases.json".to_string(),
                oracle_case_id: "ufunc_input_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c005_ufunc_metamorphic".to_string(),
                input_path: "crates/fnp-conformance/fixtures/ufunc_metamorphic_cases.json"
                    .to_string(),
                oracle_case_id: "ufunc_metamorphic_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c005_ufunc_adversarial".to_string(),
                input_path: "crates/fnp-conformance/fixtures/ufunc_adversarial_cases.json"
                    .to_string(),
                oracle_case_id: "ufunc_adversarial_cases_contract".to_string(),
            },
            FixtureEntry {
                id: "p2c005_ufunc_workflow_scenarios".to_string(),
                input_path: "crates/fnp-conformance/fixtures/workflow_scenario_corpus.json"
                    .to_string(),
                oracle_case_id: "ufunc_packet_workflow_contract".to_string(),
            },
        ],
    }
}
...
fn packet_bundle_source_files(repo_root: &Path, packet_dir: &Path) -> Vec<PathBuf> {
    vec![
        ...
        packet_dir.join("workflow_scenario_packet005_opt_e2e.jsonl"),
        packet_dir.join("workflow_scenario_packet005_opt_reliability.json"),
        packet_dir.join("workflow_scenario_packet005_opt_artifact_index.json"),
