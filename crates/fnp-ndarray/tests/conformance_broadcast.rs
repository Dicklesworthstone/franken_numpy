...
fn conformance_broadcast_shapes_same() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; print(np.broadcast_shapes((3, 4), (3, 4)))",
    )?;
    let fnp_result = fnp_ndarray::broadcast_shape(&[3, 4], &[3, 4])
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(
        format!("{fnp_result:?}"),
        numpy_result.replace("(", "[").replace(")", "]").replace(", ", ", "),
...
fn conformance_broadcast_shapes_scalar() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; print(np.broadcast_shapes((3, 4), ()))",
    )?;
    let fnp_result = fnp_ndarray::broadcast_shape(&[3, 4], &[])
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(fnp_result, vec![3, 4], "broadcast with scalar should preserve shape");
...
fn conformance_broadcast_shapes_1d_expand() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; print(np.broadcast_shapes((3, 1), (1, 4)))",
    )?;
    let fnp_result = fnp_ndarray::broadcast_shape(&[3, 1], &[1, 4])
        .map_err(|e| format!("{e:?}"))?;
...
fn conformance_broadcast_shapes_different_ndim() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; print(np.broadcast_shapes((2, 3, 4), (4,)))",
    )?;
    let fnp_result = fnp_ndarray::broadcast_shape(&[2, 3, 4], &[4])
        .map_err(|e| format!("{e:?}"))?;
...
fn conformance_fix_unknown_dim_simple() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; a = np.arange(12); print(a.reshape(3, -1).shape)",
    )?;
    let fnp_result = fnp_ndarray::fix_unknown_dimension(&[3, -1], 12)
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(fnp_result, vec![3, 4]);
    assert_eq!(numpy_result, "(3, 4)");
    Ok(())
...
fn conformance_fix_unknown_dim_first() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; a = np.arange(20); print(a.reshape(-1, 5).shape)",
    )?;
    let fnp_result = fnp_ndarray::fix_unknown_dimension(&[-1, 5], 20)
        .map_err(|e| format!("{e:?}"))?;
...
fn conformance_fix_unknown_dim_3d() -> Result<(), String> {
    let numpy_result = numpy_oracle(
        "import numpy as np; a = np.arange(24); print(a.reshape(2, -1, 4).shape)",
    )?;
    let fnp_result = fnp_ndarray::fix_unknown_dimension(&[2, -1, 4], 24)
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(fnp_result, vec![2, 3, 4]);
    assert_eq!(numpy_result, "(2, 3, 4)");
    Ok(())
...
fn conformance_fix_unknown_dim_no_unknown() -> Result<(), String> {
    let fnp_result = fnp_ndarray::fix_unknown_dimension(&[3, 4], 12)
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(fnp_result, vec![3, 4]);
    Ok(())
}
...
fn conformance_contiguous_strides_1d() -> Result<(), String> {
    ...
    let fnp_result = fnp_ndarray::contiguous_strides(&[10], 8, MemoryOrder::C)
        .map_err(|e| format!("{e:?}"))?;
...
fn conformance_contiguous_strides_scalar() -> Result<(), String> {
    use fnp_ndarray::MemoryOrder;
    let fnp_result = fnp_ndarray::contiguous_strides(&[], 8, MemoryOrder::C)
        .map_err(|e| format!("{e:?}"))?;
...
fn conformance_broadcast_shapes_single() -> Result<(), String> {
    ...
    let fnp_result = fnp_ndarray::broadcast_shapes(&[&[3, 4]])
        .map_err(|e| format!("{e:?}"))?;
    assert_eq!(fnp_result, vec![3, 4]);
    assert_eq!(numpy_result, "(3, 4)");
    Ok(())
...
fn conformance_broadcast_shapes_empty_list() -> Result<(), String> {
    let fnp_result = fnp_ndarray::broadcast_shapes(&[])
        .map_err(|e| format!("{e:?}"))?;
    assert!(fnp_result.is_empty());
    Ok(())
}
