...
| `legacy_numpy_code/numpy/numpy/_core/src/multiarray/nditer_constr.c:1463` | `npyiter_fill_axisdata` | broadcast shape synthesis, zero-stride propagation, reduction-axis detection | landed `crates/fnp-iter` operand broadcast planning core plus remaining packet-local stride-tricks integration coverage |
...
| `crates/fnp-iter/src/lib.rs:576` | `NditerPlan` / `Nditer` | implemented iterator constructor/state-machine core; packet-local stride-tricks integration coverage remains parity debt |
| `crates/fnp-iter/src/lib.rs:1324` | operand broadcast planning structs | implemented operand-shape planning boundary for broadcast/no-broadcast contracts |
| `crates/fnp-iter/src/lib.rs:1684` | `TransferContext` / `select_transfer_loop` | implemented transfer-loop decision boundary used by iterator-sensitive broadcast/view follow-up work |
...
- Implementation stage (`bd-23m.17.4`) has split responsibilities between `fnp-ndarray` (public API + shape law) and the landed `fnp-iter` iterator/transfer planning core; remaining work is packet-specific stride-tricks API integration evidence.
